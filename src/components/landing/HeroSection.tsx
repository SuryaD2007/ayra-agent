  ...
  return <div className="py-20 md:py-28 flex flex-col items-center text-center">
      <AnimatedTransition show={showTitle} animation="slide-up" duration={600}>
        ...
        <div className="flex flex-col items-center gap-4 mb-6">
          ...
          <Button size="lg" variant="outline" onClick={() => navigate('/search')} className="rounded-full px-8 py-6 text-base font-medium transition-all duration-300">
            <Search className="mr-2 h-4 w-4" />
            AI Search
          </Button>
          
          {/* Mac and Windows buttons underneath - Hidden in Electron app */}
          {!isElectron() && <div className="flex flex-wrap justify-center gap-4">
              <Button size="lg" onClick={() => handleBetaClick('mac')} className="flex items-center gap-3 rounded-2xl px-6 py-4 text-white font-semibold shadow-lg transition transform hover:-translate-y-0.5 bg-gradient-to-r from-blue-400 to-blue-600 hover:from-blue-500 hover:to-blue-700 border-0">
              ...
              <Button size="lg" onClick={() => handleBetaClick('windows')} className="flex items-center gap-3 rounded-2xl px-6 py-4 bg-gray-900 text-white font-semibold shadow-lg transition transform hover:-translate-y-0.5 hover:bg-gray-800 border-0">
          ...
          {/* Message when in Electron app */}
          {isElectron() && <p className="text-sm text-muted-foreground mt-6 text-center">
              You're already using the Ayra Desktop App.
            </p>}
        </div>

        {!isElectron() && <p className="text-sm text-muted-foreground mb-4">
            No spam. We'll email your invite when your platform opens.
          </p>}
      </AnimatedTransition>

      <BetaModal isOpen={showBetaModal} onClose={() => setShowBetaModal(false)} os={selectedOs} />
    </div>;
};