...
export const GoogleDocsConnection = () => {
  ...
  const handleSync = async () => {
    setSyncing(true);
    try {
      const { data, error } = await supabase.functions.invoke('sync-google-docs');
      
      if (error) throw error;

      toast.success(`Synced ${data.itemsSynced} documents from Google Docs`);
      await checkConnection();
    } catch (error) {
      console.error('Sync error:', error);
      toast.error('Failed to sync Google Docs');
    } finally {
      setSyncing(false);
    }
  };
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription>Import documents from Google Docs</CardDescription>
            </div>
          </div>
          {isConnected ? (
