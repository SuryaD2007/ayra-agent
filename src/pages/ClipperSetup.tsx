...
const ClipperSetup = () => {
  ...
  // Generate bookmarklet code - uses current domain dynamically
  const currentDomain = window.location.origin;
  const bookmarkletCode = `javascript:(function(){try{var u='${user?.id || 'USER_ID'}',t=document.title,l=window.location.href,s=window.getSelection().toString(),d='${currentDomain}';console.log('Ayra clipper started');if(!window.html2canvas){var c=document.createElement('script');c.src='https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js';c.onerror=function(){alert('Failed to load screenshot library. Opening clip window without screenshot.');window.open(d+'/clip?title='+encodeURIComponent(t)+'&url='+encodeURIComponent(l)+'&content='+encodeURIComponent(s)+'&userId='+u,'ayra-clipper','width=600,height=700');};c.onload=function(){console.log('html2canvas loaded');capture();};document.head.appendChild(c);}else{capture();}function capture(){var loader=document.createElement('div');loader.style.cssText='position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:rgba(0,0,0,0.9);color:white;padding:24px 32px;border-radius:12px;z-index:999999;font-family:system-ui;font-size:16px;';loader.textContent='📸 Capturing...';document.body.appendChild(loader);html2canvas(document.body,{allowTaint:true,useCORS:false,foreignObjectRendering:false,removeContainer:true,logging:false,imageTimeout:15000,ignoreElements:function(element){return element.tagName==='IFRAME'||element.tagName==='VIDEO'||element.tagName==='EMBED'||element.tagName==='OBJECT';}}).then(function(canvas){var f=document.createElement('div');f.style.cssText='position:fixed;inset:0;background:white;z-index:9999998;opacity:0.8;';document.body.appendChild(f);setTimeout(function(){document.body.removeChild(f);},300);loader.textContent='✅ Captured!';var img=canvas.toDataURL('image/png');setTimeout(function(){document.body.removeChild(loader);console.log('Opening clip window');window.open(d+'/clip?title='+encodeURIComponent(t)+'&url='+encodeURIComponent(l)+'&content='+encodeURIComponent(s)+'&userId='+u+'&screenshot='+encodeURIComponent(img),'ayra-clipper','width=600,height=700');},800);}).catch(function(e){console.error('Screenshot error:',e);document.body.removeChild(loader);window.open(d+'/clip?title='+encodeURIComponent(t)+'&url='+encodeURIComponent(l)+'&content='+encodeURIComponent(s)+'&userId='+u,'ayra-clipper','width=600,height=700');});}}catch(e){console.error('Clipper error:',e);alert('Clipper error: '+e.message+'. Opening without screenshot.');window.open('${currentDomain}/clip?title='+encodeURIComponent(document.title)+'&url='+encodeURIComponent(window.location.href)+'&userId='+'${user?.id || 'USER_ID'}','ayra-clipper','width=600,height=700');}})();`;
